...
pub async fn update(pool: &DbPool, collection: &str, id: Uuid, new_data: Value) -> Result<Content, ApiError> {
    validate_collection_name(collection)?;
...
pub async fn patch(pool: &DbPool, collection: &str, id: Uuid, partial_data: Value) -> Result<Content, ApiError> {
    validate_collection_name(collection)?;
