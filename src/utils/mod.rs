...
/// Validate a SQL-safe identifier (used for json field names and sort fields).
pub fn validate_identifier(name: &str) -> Result<(), ApiError> {
    if name.is_empty() || name.len() > 40 {
        return Err(ApiError::BadRequest(
            "field name must be 1-40 characters".into(),
        ));
    }

    if name.starts_with('_') {
        return Err(ApiError::BadRequest(
            "field names cannot start with underscore".into(),
        ));
    }
    ...
    if !valid || !name.chars().next().unwrap().is_ascii_lowercase() {
        return Err(ApiError::BadRequest(
            "field name must start with lowercase letter and contain only [a-z0-9_]".into(),
        ));
    }
